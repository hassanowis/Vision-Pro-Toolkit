...
class siftapply:
    def __init__(self, img, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
        ...
        base_image = self.generateBaseImage(grey_scaled, sigma, assumed_blur)
        ...
        self.keypoints = self.convertKeypointsToInputImageSize(keypoints)
        self.descriptors = self.generateDescriptors(keypoints, gaussian_images)
    ...
    ########################### Image pyramids ###########################
    def generateBaseImage(self, original_image, sigma, assumed_blur):
    ...
    def computeNumberOfOctaves(self, original_shape: tuple):
    ...
    def generateGaussianImages(self, original_image, num_octaves, gaussian_kernels):
                ...
                for i in range(image_border_width, first_image.shape[0] - image_border_width):
                    for j in range(image_border_width, first_image.shape[1] - image_border_width):
                        ...
                        if self.isPixelAnExtremum(first_image[i - 1:i + 2, j - 1:j + 2],
                            ...
                            localization_result = self.localizeExtremumViaQuadraticFit(i, j, image_index + 1,
                                                                                       octave_index,
                            ...
                            if localization_result is not None:
                                ...
                                keypoints_with_orientations = self.computeKeypointsWithOrientations(keypoint,
                                                                                                    octave_index,
                                                                                                    gaussian_images[
                                                                                                        octave_index][
    ...
    def localizeExtremumViaQuadraticFit(self, i, j, image_index, octave_index, num_intervals, images_in_octave,
        ...
        if abs(functionValueAtUpdatedExtremum) * num_intervals >= contrast_threshold:
            ...
            if xy_hessian_det > 0 and eigenvalue_ratio * (xy_hessian_trace ** 2) < \
                ...
                keypoint.octave = octave_index + image_index * (2 ** 8) + int(
                    round((extremum_update[2] + 0.5) * 255)) * (
                                          2 ** 16)
    ...
    def generateDescriptors(self, keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3,
        ...
        for keypoint in keypoints:
            ...
            for row_bin, col_bin, magnitude, orientation_bin in zip(row_bin_list, col_bin_list, magnitude_list,
                ...
                row_bin_floor, col_bin_floor, orientation_bin_floor = floor([row_bin, col_bin, orientation_bin]).astype(
                    int)
